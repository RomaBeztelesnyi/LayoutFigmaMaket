...
editForm.addEventListener("submit", (event) => {
    ...
    if (editForm.dataset.mode === "create") {
        if (data.title) {
            ...
            listItem.querySelector('.todo-item').setAttribute('id', `item${id-idGenerator.next().value}`)
...
function editItem(element) {
    ...
    editForm.querySelector('[name=title]').value = element.querySelector('.title').innerText
    editForm.querySelector('[name=desc]').value = element.querySelector('.desc').innerText

}
function toggleItemComplete(element)  {
